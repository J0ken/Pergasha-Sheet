import { d20Roll, damageRoll } from "../module/dice.js";
...
export default class Actor5e extends Actor {
  ...
  _prepareCharacterData(actorData) {
    const data = actorData.data;
    const boostedLimit = data.psiLimitBoost;
    ...
    // Psi-limit
    data.psilimit = getPsiLimit(level, boostedLimit);
  }
  ...
  /**
  *Return the Psi-Limit for a character
  * @param Level {Number} The character level
  * @return {Number} The character psi-limit
  */
  getPsiLimit(level, boostedLimit){
    const psiLimit;
    if(level > 0 && level < 3){
      psiLimit = 2;
